...
5. **NEVER run `wdk module add / remove / enable / disable`, `wdk network enable / disable`, or `wdk token enable / disable`** — add and remove download and install executable code that runs inside the wallet daemon; enable and disable change which built-in modules, networks, and tokens the CLI uses (and lock all wallets). If something is missing or disabled, tell the user and let them decide.
